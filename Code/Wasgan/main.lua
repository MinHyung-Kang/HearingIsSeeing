...
opt = {
    ...
    --dataset = '/idata/cs189/data/audio2image/data/ppmi/cello',     -- folder
    dataset = './../../Data/ppmi/cello',     -- folder
    display = 0,         -- port for displaying images during training / 0 = false
    ...
    loadSizeX = 128,
    loadSizeY = 128,
    lr = 5e-5,              -- initial learning rate for adam (originally 5e-5)
    name = 'onlyCello_wasgan',
    ...
    niter = 15000,             -- #  of iter at starting learning rate
    noise = 'normal',       -- uniform / normal
    nThreads  = 0,           -- #  of data loading threads to use
    ntrain = math.huge,     -- #  of examples per epoch. math.huge for full dataset
    nz = 100,               -- #  of dim for Z
    save_every = 1000,        -- save every # iterations
    init_d = '',
    init_g = '',
}
...
if opt.init_g == '' then
    ...
    print(opt.init_g .. ' loaded as generator')
end
...
if opt.init_d == '' then
    ...
    print(opt.init_d .. ' loaded as discriminator')
end
...
for epoch = 1, opt.niter do
   ...
   while i <= len_dataloader do
     ...
     stats[statIndex][2] = i-1
     stats[statIndex][3] = errD
     stats[statIndex][4] = errG
     stats[statIndex][5] = errD_real
     stats[statIndex][6] = errD_fake
   ...
   if epoch % opt.save_every == 0 then
       ...
       torch.save('checkpoints/' .. opt.name .. '_' .. epoch .. '_net_opts.t7', stats)
