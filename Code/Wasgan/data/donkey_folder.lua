...
opt.data = os.getenv('DATA_ROOT') or '/idata/cs189/data/audio2image/data/instruments/ucfimgonedir'
...
local function loadImage(path)
   ...
   if loadSize[2] ~= 0 then
       ...
       if iW < iH then
          input = image.scale(input, loadSize[2], loadSize[3] * iH / iW)
       else
          input = image.scale(input, loadSize[2] * iW / iH, loadSize[3])
       end
   end
   return input
end
...
if paths.filep(trainCache) then
   ...
   trainLoader.loadSize = {3, opt.loadSizeX, opt.loadSizeY}
   trainLoader.sampleSize = {3, sampleSize[2], sampleSize[2]}
else
   print('Creating train metadata')
   trainLoader = dataLoader{
      paths = {opt.data},
      loadSize = {3, loadSize[2], loadSize[3]},
