...
require 'optim'
require 'cudnn'
--   img_dir = '/idata/cs189/data/audio2image/data/instruments/ucfimagessubset',
--data_root = '/idata/cs189/data/audio2image/data/instruments/ucfaudionormsubset',
...
opt = {
  ...
  dataset = 'instruments',       -- imagenet / lsun / folder
   ...
   name = 'wasganFinal_72by72',
   noise = 'normal',       -- uniform / normal
   init_g = '',
   init_d = '',
   --init_g = '/ihome/mkang/cs89/code/audio2image/wasgan/checkpoints/onlyCello_wasgan_64by64_from10000_10000_net_G.t7',
   --init_d = '/ihome/mkang/cs89/code/audio2image/wasgan/checkpoints/onlyCello_wasgan_64by64_from10000_10000_net_D.t7',
   use_cudnn = 1,
...
if opt.init_g == '' then
  ...
  fcG = nn.Sequential()
  fcG:add(nn.Linear(opt.auxSize,opt.na)) -- aux to aux features
  ...
  netG:add(SpatialFullConvolution(ngf*8,ngf*4,1,1))
  netG:add(SpatialBatchNormalization(ngf*4)):add(nn.ReLU(true))
  netG:apply(weights_init)

  netG_sub = torch.load(opt.init_g)
  netG_sub:remove(1)
  netG_sub:remove(1)
  netG_sub:remove(1)
...
if opt.init_d == '' then
    ...
    convD:add(SpatialBatchNormalization(ndf * 4)):add(nn.LeakyReLU(0.2, true))
    -- state size: (ndf*4) x 8 x 8
    convD:add(SpatialConvolution(ndf * 4, ndf * 8, 4, 4, 2, 2, 1, 1))
    convD:add(SpatialBatchNormalization(ndf * 8)):add(nn.LeakyReLU(0.2, true))
  ...
  netD:apply(weights_init)
   ...
   convD = torch.load(opt.init_d)
   convD:remove(13)
   convD:remove(12)
   --convD.modules[9] = nil
   --convD.modules[10]= nil
   --collectgarbage()
